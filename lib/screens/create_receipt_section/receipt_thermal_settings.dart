...
//
// CASHIER NAME TOGGLE PASS (this update): added `showCashierName` /
// `onShowCashierNameChanged` param pair (forwarded straight through from
// create_receipt_screen.dart via receipt_step_customise.dart — see both
// files' matching passes). New toggle row placed directly above the
// "Staff & Terminal" field row, right next to the Cashier Name field it
// controls — deliberately not grouped with the other section-visibility
// toggles at the top of this file, since those all gate whole sections
// while this one gates a single field.
//
// CHARACTER-LIMIT HARDENING PASS (earlier update): every field on this
...
class ReceiptThermalSettingsSection extends StatelessWidget {
  ...
  // CASHIER NAME TOGGLE PASS: new — gates the Cashier Name field in the
  // "Staff & Terminal" section below, rendered right next to that field
  // rather than grouped with the section-visibility toggles above it.
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        // CASHIER NAME TOGGLE PASS: sits directly above the field it
        // controls, rather than up with the section-visibility toggles —
        // this one gates a single field, not a whole block.
        _toggleRow(context, 'Show cashier name on receipt', showCashierName, onShowCashierNameChanged),
