// lib/screens/create_receipt_section/step_create_receipt/step_create_receipt.dart
//
// SAVED-ITEMS FILTERS PASS (this update): adds a search field +
// Recent/A-Z/Z-A sort selector to the "Saved Receipts" list, matching
// the same treatment already applied to the Templates step
// (receipt_step_template.dart) — new _DraftSearchField /
// _DraftSortSelector widgets at the bottom of this file, matching
// relevance against the draft's display name, receipt number, and
// client name. The library's previous hardcoded
// `_library.length - 1 - displayIdx` reversal (via List.generate) is
// gone — the card list now iterates `_visibleIndices`, which reproduces
// that exact same newest-first order under the default "Recent" sort
// mode, so nothing changes visually until a person actually searches or
// picks a different sort.
//
// CREATE-RECEIPT UI PARITY PASS (earlier): _ReceiptDraftCard renders
// the draft's own container logo (SavedReceiptDraft.logoPath, set via
// create_receipt_bottom_sheet.dart's "Container Logo" section) exactly
// the way Quote's/Invoice's own draft cards do — a real
// SharedLogoThumbnail when a logo is set, else the same rotated-square
// initial-letter fallback mark (respecting logoShowInitial/
// logoInitialLetter), else a plain icon as the last resort.
// _ReceiptDraftCardFallbackMark mirrors Quote's/Invoice's own
// fallback-mark widget exactly, adapted to SavedReceiptDraft's flat
// logo fields.
//
// PATH FIX PASS (earlier): this file's real home is
// lib/screens/create_receipt_section/step_create_receipt/ — two levels
// under lib/, matching Invoice's step_create_invoice/ and Quote's
// step_create_quote/ folders exactly.
//
...
// SAVED-ITEMS FILTERS PASS: sort modes for the saved-receipt list,
...
class _StepCreateReceiptState extends State<StepCreateReceipt> {
  ...
  // receipt number, and client name. 3 means "doesn't match" and gets
  // filtered out.
  int _relevance(int i, String q) {
    final draft = _library[i];
    final d = draft.data;
    final name = draft.displayName.toLowerCase();
    final receiptNumber = d.receiptNumber.toLowerCase();
    final client = d.clientName.toLowerCase();
    if (name.startsWith(q)) return 0;
    if (name.contains(q)) return 1;
    if (receiptNumber.contains(q) || client.contains(q)) return 2;
    return 3;
  }
      ...
      children: [
        ...
        if (!_loading && _library.isNotEmpty) ...[
          ...
          if (_showLibraryPanel) ...[
            ...
            _DraftSearchField(
              ...
              accent: accent,
              hintText: 'Search saved receipts…',
        ...
        if (!_loading && _showLibraryPanel && _library.isNotEmpty) ...[
          ...
          if (visible.isNotEmpty)
            ...visible.map((i) => _ReceiptDraftCard(
                  draft: _library[i],
...
// =============================================================================
// SAVED-ITEMS FILTERS PASS: search field for the saved-receipt list.
...
// saved-receipt list. Same shape as the Templates step's own sort
// selector.
// =============================================================================
...
class _ReceiptDraftCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // CREATE-RECEIPT UI PARITY PASS: real thumbnail when the container
    // has a logo set, else the same rotated-square fallback mark Quote's/
    // Invoice's own draft cards use (or a plain icon as the last resort
    // when logoShowInitial is off).
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              // Logo / fallback mark — see CREATE-RECEIPT UI PARITY PASS
              // above.
              Container(
                width: 46,
                height: 46,
                ...
                child: hasLogo
                    ...
                    : (draft.logoShowInitial
                        ? _ReceiptDraftCardFallbackMark(draft: draft, accent: accent)
                        : Icon(
...
// =============================================================================
// _ReceiptDraftCardFallbackMark — rotated-square initial mark shown when
// a draft has no container logo set and logoShowInitial is on. Mirrors
// Quote's/Invoice's own fallback-mark widget exactly, adapted to
// SavedReceiptDraft's flat logo fields (and draft.name / the receipt's
// client name as the letter source) instead of BusinessInfo's nesting.
// =============================================================================

class _ReceiptDraftCardFallbackMark extends StatelessWidget {
  final SavedReceiptDraft draft;
  final Color accent;
  const _ReceiptDraftCardFallbackMark({required this.draft, required this.accent});
  ...
  Widget build(BuildContext context) {
    ...
    final initial = letter.isNotEmpty
        ...
        : (source.isNotEmpty ? source[0].toUpperCase() : 'R');
    return Stack(
