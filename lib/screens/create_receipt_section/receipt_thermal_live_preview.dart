...
// lib/screens/create_receipt_section/receipt_thermal_live_preview.dart
//
// PER-ITEM TAX/DISCOUNT PASS (this update): added breakdown rows for
// d.itemTaxExtraByName / d.itemDiscountExtraByName (new getters on
// ReceiptData — see that file's PER-ITEM TAX/DISCOUNT PASS) right after
// the existing whole-receipt Discount/Tax rows, one row per distinct
// item-tax/discount name, mirrored from how these getters already sum
// correctly. TOTAL now reads d.amountPaid, which already folds these
// extras in at the model level, so no other change was needed here to
// get the right final number — only the visible breakdown rows are new.
// Every receipt saved before this pass has zero on every item's
// itemTaxRate/itemDiscountRate (the Create step never wrote to them
// until now), so these new rows render nothing and TOTAL is unchanged
// for every existing persisted receipt.
//
// CASHIER NAME TOGGLE PASS (earlier): the Cashier row was previously
// shown whenever d.cashierName was non-empty, with no way to hide it.
// Now also gated on the new d.showCashierName toggle (see
// receipt_data.dart's matching pass) — mirrors how every other metadata
// row here (Receipt No, Date, POS ID) is gated by its own show* field.
//
// CURRENCY DISPLAY PASS (earlier): the old _currencySymbol(code) helper
// was a hardcoded code -> symbol switch that completely ignored
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // CASHIER NAME TOGGLE PASS: now also gated on d.showCashierName,
          // matching every other metadata row's show*-gated pattern.
          if (d.showCashierName && d.cashierName.isNotEmpty) metaRow('Cashier:', d.cashierName),
          ...
          ...d.lineItems.map((item) => Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      flex: 3,
                      child: Text('$sym${item.lineNetTotal.toStringAsFixed(2)}',
          ...
          // PER-ITEM TAX/DISCOUNT PASS: one row per distinct item-tax/
          // discount name — see receipt_data.dart's
          // itemTaxExtraByName/itemDiscountExtraByName getters.
