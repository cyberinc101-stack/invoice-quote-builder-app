// lib/screens/create_quote_section/step_create_quote/create_quote_saved_line_items_widgets.dart
//
// PRICE RANGE FILTER PASS (this update): CreateQuoteSavedLineItemsPanel
// gained a Min/Max price filter (CreateQuoteSavedItemsPriceRangeFilter,
// create_quote_saved_items_filter_bar.dart) sitting directly under the
// sort dropdown, mirroring Invoice's/Receipt's identical addition. Two
// plain TextEditingControllers (_minPriceCtrl/_maxPriceCtrl) — blank
// means "no bound" on that side. An entry must match the search text
// AND fall within [min, max] (each side only applied when that field
// actually parses to a number) to be included. Sorting is applied after
// both filters. Same `library.length > 4` gate as the search field/sort
// dropdown, and it collapses along with everything else when the
// panel's header is tapped shut.
//
// COUNT BADGE PASS (earlier): the "Saved Items" header badge now
// shows "X/200" (X/kMaxSavedQuoteLineItems) instead of just the bare
...
// against their cap). Previously this badge showed only the raw
// library length with no visible ceiling.
//
// NEW FILE (earlier) — CREATE-QUOTE PARITY PASS: mirrors
// create_invoice_saved_line_items_widgets.dart exactly, adapted to
// SavedQuoteLineItem (quote_data.dart) instead of SavedInvoiceLineItem —
// own SharedPreferences key ('quote_saved_single_line_items') so a
// saved quote line item never mixes with Invoice's single-item library.
// This is the browsing UI for INDIVIDUAL saved line items, as opposed to
// quote_saved_items_widgets.dart's QuoteSavedItemSets, which handles
// BUNDLES of several items at once — kept as a fully separate widget/
// file, same split Invoice already has between its two saved-item
// panels.
//
// Each card carries a radio-style checkbox reflecting whether THIS quote
// currently includes an item sourced from that saved container (tracked
// via LineItem.sourceSavedId — see create_quote_bottom_sheet.dart's
// _toggleSavedLineItem). Tapping toggles it live; each card also has its
// own edit (rename + adjust description/qty/price/unit/tax/discount) and
// delete actions, plus a search field + sort dropdown once the library
// grows past 4 items.
...
import '../../../models/quote_data.dart' show SavedQuoteLineItem;
...
import 'create_quote_saved_items_filter_bar.dart'
    show
        QuoteSavedLineItemSortOption,
        CreateQuoteSavedItemsSortDropdown,
        CreateQuoteSavedItemsPriceRangeFilter;

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
const int kMaxSavedQuoteLineItems = 200;
const _kPrefSavedQuoteLineItemList = 'quote_saved_single_line_items';

// ---------------------------------------------------------------------------
// Persistence helpers — shared by this panel and the parent bottom
// sheet's bookmark-save action.
// ---------------------------------------------------------------------------
Future<void> persistSavedQuoteLineItems(List<SavedQuoteLineItem> list) async {
  final prefs = await SharedPreferences.getInstance();
  await prefs.setString(
    _kPrefSavedQuoteLineItemList,
    jsonEncode(list.map((s) => s.toJson()).toList()),
  );
}

Future<List<SavedQuoteLineItem>> loadSavedQuoteLineItems() async {
  final prefs = await SharedPreferences.getInstance();
  final raw = prefs.getString(_kPrefSavedQuoteLineItemList);
  if (raw == null || raw.isEmpty) return [];
  try {
    return (jsonDecode(raw) as List)
        .map((e) => SavedQuoteLineItem.fromJson(e as Map<String, dynamic>))
        .toList();
  } catch (_) {
    return [];
  }
...
class CreateQuoteSavedLineItemsPanel extends StatefulWidget {
  final List<SavedQuoteLineItem> library;
  final Set<String> includedIds;
  final String currencySymbol;
  final Color accent;
  final ValueChanged<SavedQuoteLineItem> onToggleInclude;
  final void Function(int index, SavedQuoteLineItem updated) onEdit;
  final void Function(int index) onDelete;

  const CreateQuoteSavedLineItemsPanel({
    super.key,
  ...
  @override
  State<CreateQuoteSavedLineItemsPanel> createState() =>
      _CreateQuoteSavedLineItemsPanelState();
}

class _CreateQuoteSavedLineItemsPanelState
    extends State<CreateQuoteSavedLineItemsPanel> {
  String _query = '';
  bool _collapsed = false;
  QuoteSavedLineItemSortOption _sort = QuoteSavedLineItemSortOption.dateNewest;
  ...
  void _openEditSheet(int index) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _EditSavedQuoteLineItemSheet(
        existing: existing,
  ...
  void _applySort(List<MapEntry<int, SavedQuoteLineItem>> entries) {
    switch (_sort) {
  ...
  Widget build(BuildContext context) {
    ...
    final entries = <MapEntry<int, SavedQuoteLineItem>>[];
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    '${widget.library.length}/$kMaxSavedQuoteLineItems',
                    style: TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.w700,
                      color: widget.library.length >= kMaxSavedQuoteLineItems
        ...
        if (!_collapsed) ...[
          ...
          if (entries.isEmpty)
              ...
              child: Center(
                child: Text(
                  widget.library.isEmpty
                      ? 'No saved items yet. Save one from the item card below.'
            ...
            ...entries.map((e) {
              ...
              return _SavedQuoteLineItemCard(
                saved: saved,
...
class _SavedQuoteLineItemCard extends StatelessWidget {
  final SavedQuoteLineItem saved;
...
class _EditSavedQuoteLineItemSheet extends StatefulWidget {
  final SavedQuoteLineItem existing;
  final String currencySymbol;
  final Color accent;
  final ValueChanged<SavedQuoteLineItem> onSaved;

  const _EditSavedQuoteLineItemSheet({
  ...
  @override
  State<_EditSavedQuoteLineItemSheet> createState() => _EditSavedQuoteLineItemSheetState();
}

class _EditSavedQuoteLineItemSheetState extends State<_EditSavedQuoteLineItemSheet> {
